#pragma once
#ifndef PYJS_MPOOL_HPP
#define PYJS_MPOOL_HPP
...
inline py::dict transaction_to_py(Transaction* transaction)
{
    ...
    auto as_str = [&pool](Solvable* s) {
        const char* name = pool_id2str(pool, s->name);
        const char* evr = pool_id2str(pool, s->evr);
        const char* build_string;
        build_string = solvable_lookup_str(s, SOLVABLE_BUILDFLAVOR);
        std::string res_string(name);
        res_string.append("-");
        res_string.append(evr);
        res_string.append("-");        
        res_string.append(build_string);
        return res_string;
    };
    ...
    for (int i = 0; i < transaction->steps.count; i++)
    {
        ...
        switch (ttype)
        {
            ...
            {
                std::cout << "Removing " << as_str(s) << std::endl;

                remove_list.append(as_str(s));
                s2 = transaction->pool->solvables + transaction_obs_pkg(transaction, p);
                s2 = pool_id2solvable(pool, i2);
                std::cout << "Installing " << as_str(s2) << std::endl;
                install_list.append(as_str(s));
                break;
            }
            ...
            {
                std::cout << "Installing " << as_str(s) << std::endl;
                install_list.append(as_str(s));
                break;
            }
